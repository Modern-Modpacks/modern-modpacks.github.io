<script lang="ts">
    ...
    import { canDeleteComment, closeBlogpost, getContributorAvatar, sendGithubApiRequest } from "$lib/scripts/utils";
...
<p>${blogpost.ghdata?.created.author.name}</p>`,
    ...
    <p>${blogpost.ghdata?.edited.author.name}</p>`,
                            ...
                            <h1 class="text-left mb-2">{blogpost.metadata.title} <span class="opacity-35">{blogpost.metadata.subtitle}</span></h1>
                        ...
                        <div class="
                            ...
                            [&_*]:max-w-full [&_*]:[grid-column:2/3] [&_*]:h-fit desktop:[&_*]:py-1 mobile:[&_*]:py-3
                        ...
                        <div class="flex flex-col gap-12 desktop:py-16 mobile:pt-12 mobile:pb-8 px-96 mobile:px-12" bind:this={commentsElement}>
                            <span class="relative flex items-center gap-2 w-full h-64 mobile:h-48 pt-2 pb-4 desktop:pr-4 {ghUserData ? "pl-2" : "pl-6"} rounded-xl {$lightMode ? "bg-secondary-light" : "bg-secondary-dark"} shadow-[#000000aa] shadow-2xl">
                                {#if ghUserData}
                                ...
                                {#if ghUserData}
                                    ...
                                    <button class="group w-12 h-12 mobile:w-9 mobile:h-9 aspect-square p-2 mobile:p-1 self-end {$lightMode ? "bg-text-light" : "bg-text-dark"} rounded-full" on:click={async () => {
                                        if (!comment) return
                            ...
                            <div class="flex flex-col gap-8 desktop:px-16">
                                {#each blogpost.comments?.toReversed() ?? [] as c}
                                    <span class="block w-full h-fit p-4 rounded-xl {$lightMode ? "bg-secondary-light" : "bg-secondary-dark"}">
                                        <span class="relative flex items-center gap-2 [&>b]:text-xl mobile:[&>b]:text-sm">
                                            ...
                                            {#if ghUserData && canDeleteComment(ghUserData.login, c.author.name)}
                                                <button class="group absolute right-2 mobile:right-0 mobile:w-8 mobile:h-8 p-2 rounded-full {$lightMode ? "bg-text-light" : "bg-text-dark"}" title="{$_("ui.deletecomment")}" on:click={async () => {
                                                    await fetch(`/api/blogComments/delete?id=${id}&token=${$ghApiKey}`, {
                                                        method: "POST",
                                                        body: JSON.stringify(c)
                                                    })
                                                    await fetchSocialData()
                                                }}>
                                                    <Trash_2 color="{$lightMode ? "#ffffff" : "#000000"}" class="w-full h-full desktop:motion-safe:group-hover:rotate-[30deg] duration-200" />
                                                </button>
                                            {/if}
                                        </span>
